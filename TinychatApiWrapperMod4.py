...
class TinychatApiWrapper:
    ...
    def connect_token(room, console):
        try:
            web_session = web.WebSession()
            response = TinychatApiWrapper._get_connect_token(room, web_session)
            if response is not None:
                return TinychatApiWrapper._parse_connect_token_response(response, console)
        except Exception as e:
            log.error("Error in connect_token: {}".format(e))
        return None

    @staticmethod
    def _get_connect_token(room, web_session):
        url = 'https://tinychat.com/api/v1.0/room/token/{}'.format(room)
        return web_session.get(url, as_json=True)
