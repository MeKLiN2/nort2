...
class TinychatApi:
    ...
    def connect_token(cls, room, console):
        ...
        if 'result' not in response_json:
            console.write("Unexpected response format. Missing 'result' key.", Color.B_RED)
            return None

        if response_json['result'] != 'success':
            console.write("Failed to retrieve connect token. Result: {}".format(response_json['result']), Color.B_RED)
            return None
    ...
    def user_info(cls, account, console):
        ...
        if response_json['result'] != 'success':
            console.write("Failed to retrieve user info. Result: {}".format(response_json['result']), Color.B_RED)
            return None
