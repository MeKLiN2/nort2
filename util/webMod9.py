...
class WebSession(object):
    ...
    def _parse_token(self, response=None):
        ...
        if response is None:
            response = self.get(url=token_url, referer=token_url)
    ...
    def is_cookie_expired(self, cookie_name):
        """
...
def get(url, referer=None, proxy=None, as_json=False):
    ...
    session = WebSession()  # Create a new instance of WebSession

    try:
        response = session.get(url, headers=headers, proxies={'http': proxy, 'https': proxy} if proxy else None)
